...
The `as_survey_rep()` function creates a survey object with replicate weights. 
...
Viewing the design object, we confirm RECS is an "unstratified cluster jacknife (JK1) with 60 replicates and MSE variances."
...
Calculate the estimated average cost of electricity (`DOLLAREL`) in the United States using `summarize` and `survey_mean`:

```r
recs_des %>%
  summarize(elec_bill = survey_mean(DOLLAREL, # <1>
                                    vartype = c("se", "ci"))) # <2>
```

1. Calculate mean of `DOLLAREL` variable
2. Include standard error and confidence interval
...
What is the estimated average nighttime temperature U.S. households set their homes to during the summer (`SummerTempNight`)?
...
recs_des %>%
  svyttest(
    formula = SummerTempNight - 68 ~ 0, # <1>
    design = ., # <2>
    na.rm = TRUE
  )
```

1. Formula to test if the true mean of `SummerTempNight` variable minus 68&deg;F is equal to 0
2. Dot notation `.` that passes the `recs_des` object into the `design` argument

## Results: one-sample t-test

See if the average U.S. household sets its temperature at a value different from 68&deg;F using `svyttest()`:
...
Test if the electricity expenditure is significantly different between homes with and without air-conditioning `svyttest()`:

```r
recs_des %>%
  svyttest(
    formula = DOLLAREL ~ ACUsed, # <1>
    design = ., 
    na.rm = TRUE
  )
```

1. Formula with electricity expenditure on the left and air-conditioning usage on the right

## Results: two-sample t-test

See if the electricity expenditure is significantly different between homes with and without air-conditioning `svyttest()`:
...
Create a table for estimated average household electricity bill: 
...
# The End {background-color='{{< brand color secondary >}}'}
